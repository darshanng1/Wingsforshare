      ...
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16">
              ...
              <div className="relative h-full min-h-[400px] md:min-h-[500px] rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl">
                ...
                <div className="absolute bottom-6 left-6 md:bottom-12 md:left-12 p-2 max-w-2xl">
                   <div className="flex items-center gap-4 mb-6">
                    <span className="px-4 py-2 bg-accent rounded-xl text-white text-[10px] font-black uppercase tracking-widest inline-block">
                      Featured Insight
                    </span>
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/40">{blogs[0].date}</span>
                   </div>
                  <h2 className="text-3xl md:text-5xl lg:text-6xl font-display font-black text-white leading-[1.1] group-hover:text-accent transition-colors">
                    {blogs[0].title}
                  </h2>
                </div>
              </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-10 md:gap-12">
            <div className="p-8 md:p-12 rounded-[2.5rem] bg-card-bg border border-card-border backdrop-blur-3xl shadow-2xl flex flex-col h-full justify-between">
              <div>
                <h3 className="text-2xl font-black text-white mb-6 tracking-tight">Stay in the Loop</h3>
                ...
                <div className="relative group/input">
                  ...
                  <div className="relative flex">
                    <input 
                      ...
                      className="w-full pl-6 pr-16 py-4 rounded-2xl bg-bg border border-card-border focus:outline-none focus:border-accent text-white placeholder:text-text-secondary/30 transition-all font-medium"
                    />
              ...
              <div className="mt-12 pt-10 border-t border-white/5">
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.from(new Set(blogs.map(b => b.category))).map(cat => (
                    <button key={cat} className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black text-text-secondary hover:text-white hover:border-accent uppercase tracking-widest transition-all">
                      {cat}
                    </button>
                  ))}
                </div>
            ...
            >
              <Link to={`/blog/${blog.slug}`} className="block">
                ...
                <div className="px-4 space-y-4">
                  ...
                  <h3 className="text-2xl font-bold leading-snug text-white group-hover:text-accent transition-colors">
                    {blog.title}
                  </h3>
